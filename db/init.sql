...
(
  ...
  formula VARCHAR(255),
  mass FLOAT,
  id_equivalent INTEGER REFERENCES molecule (id)
  );
...
SELECT molecule.id AS id, molecule.name AS name, 
  molecule.formula AS formula, 
  molecule.mass AS mass,
  equivalent.name AS equivalent
FROM molecule
LEFT JOIN molecule as equivalent 
     ON molecule.id_equivalent = equivalent.id
ORDER BY molecule.id;

-- Function to get all synonyms of a molecule
CREATE OR REPLACE FUNCTION func_synonym_molecule(in_id_mol INTEGER)
RETURNS TABLE(synonym VARCHAR(255)[]) AS $$
BEGIN
    RETURN QUERY
    SELECT 
